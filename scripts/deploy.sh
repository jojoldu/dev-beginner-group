...
BUILD_FILE_PATH=$(ls ~/app/build/dev-beginner-group-web/build/libs/*.jar)
...
echo "> $IDLE_SET 배포"
nohup java -jar -Dspring.profiles.active=$IDLE_SET $IDLE_APPLICATION 2>&1 > ~/app/nohup/$IDLE_SET.nohup.out &
